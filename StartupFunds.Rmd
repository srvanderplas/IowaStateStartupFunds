...
tmp <- hires[,c("Name", "College", "Dept1", "Dept2", "Year", "ProfRank", "Total.Cost")]
tmp$Name <- str_to_title(tmp$Name)
citations <- merge(tmp, citations, by.x="Name", by.y="author", all.x=T, all.y=T)
names(citations)[names(citations)=="year"] <- "year.pub"

citations$ProfRankFac <- citations$ProfRank %>% 
...
citations$CollegeFac <- citations$College %>% factor(labels=c("AGLS", "Bus", "Des", "Engr", "Hum Sci", "LAS", "Vet Med"))

citations$YearsAtISU <- 2015 - citations$Year

# Remove citations from more than one year prior to hire
citations <- subset(citations, year.pub+1>=Year)

citations.sum <- citations %>% group_by(Name, CollegeFac, Dept1, Dept2, Year, YearsAtISU, year.pub, ProfRankFac, Total.Cost) %>% summarize(total=length(type), articles=sum(type=="article"), books=sum(type=="book"), other=sum(type=="other"))

```

```{r GScholarCollege, fig.width=6, fig.height=4, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, !is.na(CollegeFac))) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_smooth(aes(group=Dept1), se=F, alpha=.25) + 
  facet_wrap(~CollegeFac, ncol=4) + 
  xlab("Years between hire and publication") + ylab("Papers Published/Year") + 
...
From this broad overview, we see that publications in many departments peak at approximately 5 years, and then tail off slightly by year 10. 

### Analysis of Publications by College

```{r GScholarLAS, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="LAS")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=6) + 
  xlab("Years between hire and publication") + ylab("Papers Published/Year") + 
  ggtitle("Yearly Publications") +
  theme_bw() + theme(legend.position="bottom", legend.direction="vertical") + 
  scale_x_continuous(breaks=c(-5, 0, 5, 10))
...
In some LAS departments, like Computer Science, publication frequency does not appear to be impacted by career stage. In other departments, such as Chemistry, publications increase over time, and then may decrease after 5-6 years (tenure). Departments such as Music do not seem to have much publication pressure (at least not which would appear on Google Scholar). In the statistics department, most hires publish at least 4 papers per year throughout their careers (perhaps 2-3 initially), but after 3-5 years, some professors begin publishing at a much higher rate, likely due to increased collaboration. 

```{r GScholarAGLS, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="AGLS")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=6) + 
...
In Ag & Life Sciences, publications seem to primarily increase over time, perhaps due to the laboratory-intensive nature of work in many of the AGLS departments. 


```{r GScholarBus, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="Bus")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=3) + 
  xlab("Years between hire and publication") + ylab("Papers Published/Year") + 
  ggtitle("Yearly Publications") +
  theme_bw() + theme(legend.position="bottom", legend.direction="vertical") + 
  scale_x_continuous(breaks=c(-5, 0, 5, 10))
...
In the business college, many individuals seem to follow the same publishing trajectory observed in other departments: publications increase from years 0-5(ish) and then decrease, however, there are relatively few individuals in this data, suggesting that the observation should be treated with some skepticism. 

```{r GScholarDes, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="Des")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=3) + 
...
The College of Design seems to have relatively few publications per person, per year (as indexed by Google Scholar). This scarcity of data makes drawing concludions difficult. 

```{r GScholarEngr, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="Engr")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=3) + 
...
The College of Engineering seems to have a slowly increasing publication rate for individuals (with considerable variability in any single year), across all departments.


```{r GScholarHumSci, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="Hum Sci")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=3) + 
...
In the College of Human Sciences, yearly variability in publications seems to outweigh any temporal trends in the number of publications per year after hire. 

```{r GScholarVet, fig.width=6, fig.height=5, dpi=300}
ggplot(aes(x=year.pub-Year+.5, y=total), data=subset(citations.sum, CollegeFac=="Vet Med")) +
  geom_vline(xintercept=0) + 
  geom_jitter(size=1, shape=1, position=position_jitter(width=.5, height=0), alpha=.3) + 
  geom_line(aes(group=Name), alpha=.5) + 
  facet_wrap(~Dept1, ncol=3) + 
...
In the College of Veterinary Medicine, publications seem to be confined to certain individuals, with most individuals publishing less than 5 papers per year that were indexed by Google Scholar. 
...
startup.quantiles <- isuHires %>% group_by(CollegeFac) %>% summarize(Q1=quantile(Total.Cost, .25, type=8), Q2=quantile(Total.Cost, .5, type=8), Q3=quantile(Total.Cost, .75, type=8), Q4=quantile(Total.Cost, 1, type=8)) %>% melt(id.vars=1, measure.vars=2:ncol(.), variable.name="Quantile", value.name="Total.Cost")

# Rank individual faculty startup packages by the quartile of their respective college. 
isuHires <- isuHires %>% group_by(CollegeFac) %>% mutate(TotalCostFac = as.numeric(cut(Total.Cost, breaks=quantile(Total.Cost, probs=0:4/4))))
